package br.com.christianrodrigues.controller;
...
public class PrimeiraController {
    ...
    public String primeiroMetodo(@PathVariable String id) {
        return "O parâmetro que você enviou foi " + id;
    }
    ...
    public ResponseEntity<Object> metodoResponseEntity(@PathVariable Long id) {
        var usuario = new Usuario("chrisrodrigues");

        if (id > 5) {
            return ResponseEntity.status(HttpStatus.OK).body(usuario);
        } else {
            return ResponseEntity.badRequest().body("Num. menor que 5");
        }
    }
